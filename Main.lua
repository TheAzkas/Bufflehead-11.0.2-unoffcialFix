...
function MOD.OptionsPanel()
	if not optionsLoaded and not optionsFailed then
		optionsLoaded = true
		local loaded, reason = C_AddOns.LoadAddOn(MOD_Options) -- try to load the options panel on demand
		if not loaded then
...
function MOD:Button_OnAttributeChanged(k, v)
	...
	if k == "index" then -- update a buff or debuff
		Auradata = C_UnitAuras.GetAuraDataByIndex(unit, v, filter)
		name = Auradata.name
		icon = Auradata.icon
		count = Auradata.applications
		btype = Auradata.dispelName
		duration = Auradata.duration
		expire = Auradata.expirationTime
		if name then
			show = true
